	...
	function instance:collision( event )
		...
		if phase == "began" and other.name == "hero" and not other.isDead then
			...
			transition.to( self.fill.effect, { time = 666, exposure = -5, onComplete = function()
				fx.fadeOut( function()
					composer.gotoScene( "scene.refreshLevel7Sub", { params = { map = self.map, score = scene.score:get() } } )
				    --composer.gotoScene( "scene.level7Sub", { params = { map = self.map, score = scene.score:get() } } )
				end )
			end } )
		end
	end
